...
- **D-03:** RPCs familia publicnode mainnet en las 3 chains: `arbitrum-one-rpc.publicnode.com`, `base-rpc.publicnode.com`, `polygon-bor-rpc.publicnode.com` (decisión de la discusión conjunta 19-08: no hay key de Alchemy disponible; el RPC de wakeup/vault-aggregator era Sepolia y no sirve). Transports `http()` configurables por env por si aparece una key.
...
### Confirmado en la discusión conjunta (19-08)
- El usuario TIENE ARGt real y gas en las 3 chains: la demo es 100 % mainnet, sin mocks de fondos.
- UI: neobanco pulido con identidad propia (el UI-SPEC de esta fase lo baja a contrato: shadcn new-york, Geist, mobile-first). El nombre propio se elige en fase 5; hasta entonces, tokens neutros.
- La wallet M2 (`0x13B5...B992`) es deployer/publisher/bóveda de fases 2-3; irrelevante para esta fase salvo que las addresses viven en el módulo de config único (D-10).
